...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['account_to_delete'])) {
    ...
    $delete_stmt = $pdo->prepare("DELETE FROM accounts WHERE user_id = ? AND account_name = ?");
    $delete_stmt->execute([$user_id, $account_to_delete]);

    // Optionally, delete transactions related to this account
    $delete_transactions_stmt = $pdo->prepare("DELETE FROM transactions WHERE user_id = ? AND account_type = ?");
    $delete_transactions_stmt->execute([$user_id, $account_to_delete]);

    // Redirect back to dashboard with a success message
    header("Location: personal_dashboard.php?message=Account deleted successfully!");
    exit();
}
?>
